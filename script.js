...
const products = [
    ...
    // 👇 HANDLOOM LUNGI WITH BRAND, STOCK & 4 IMAGES
    { 
        id: 10, 
        name: "Handloom Lungi", 
        price: 599, 
...
// =====================
// RENDER PRODUCTS (WITH BRAND + STOCK + GALLERY)
// =====================
function renderProducts(productList = products) {
    ...
    grid.innerHTML = productList.map(p => `
        <div class="product-card">
            ...
            <h3>${p.name}</h3>
            ${p.brand ? `<p class="brand">🏷️ Brand: ${p.brand}</p>` : ''}
            ${p.quantity ? `<p class="stock">📦 Stock: ${p.quantity}</p>` : ''}
