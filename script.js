...
// =====================
// WHATSAPP SETUP (SAB SET HAI)
// =====================
const YOUR_PHONE = "919905823282"; // ✅ Tumhara number
const YOUR_API_KEY = "jzksZKgAmS2C"; // ✅ TextMeBot API Key
...
if (orderForm) {
    orderForm.addEventListener('submit', function(e) {
        ...
        const message = `🛍️ New Order From Araria Mart!%0A%0A👤 Name: ${name}%0A📞 Phone: ${phone}%0A📍 Address: ${address}%0A💳 Payment: ${payment}%0A💰 Total: ₹${total}%0A%0A📦 Items: ${cart.map(item => item.name + ' x' + item.quantity).join(', ')}`;
        
        fetch(`https://api.textmebot.com/send.php?recipient=${YOUR_PHONE}&apikey=${YOUR_API_KEY}&text=${message}`, {
            method: 'POST'
        }).then(() => {
            alert(`✅ Order Placed! WhatsApp notification sent.`);
        }).catch(() => {
            alert(`✅ Order Placed! WhatsApp notification failed.`);
        });
