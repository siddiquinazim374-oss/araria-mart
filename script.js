...
const products = [
    ...
    // 👇 HIJABI DUPATTA (Image link update karein)
    { 
        id: 11, 
        name: "Hijabi Dupatta", 
        price: 249, 
...
if (orderForm) {
    orderForm.addEventListener('submit', function(e) {
        ...
        // ===== HAR PRODUCT KI IMAGE KE SATH LIST =====
        let productList = '';
        cart.forEach((item, index) => {
            productList += `${index + 1}. ${item.name}\n`;
            productList += `   Price: ₹${item.price} x ${item.quantity} = ₹${item.price * item.quantity}\n`;
            productList += `   Image: ${item.image}\n\n`;
        });
        ...
        fetch('https://formspree.io/f/' + YOUR_EMAIL_FORM_ID, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(emailData)
        }).then(() => {
            alert(`✅ Order Placed! Email sent with all product images.`);
        }).catch(() => {
            alert(`✅ Order Placed! Email sent but image links may not work.`);
        });
